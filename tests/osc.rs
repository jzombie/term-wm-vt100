...
fn clipboard() {
    ...
    impl term_wm_vt100::Callbacks for Clipboard {
        ...
        fn paste_from_clipboard(&mut self, _: &mut term_wm_vt100::Screen, ty: &[u8]) {
            self.pasted.push(ty.to_vec());
        }

        fn unhandled_osc(&mut self, _: &mut term_wm_vt100::Screen, params: &[&[u8]]) {
            panic!("unhandled osc: {params:?}");
        }
    }

    let mut parser =
        term_wm_vt100::Parser::new_with_callbacks(24, 80, 0, Clipboard::default());
